...
ENT.JumpSound = Sound("npc_dagoth/jump.mp3")
ENT.JumpHighSound = Sound("npc_dagoth/spring.mp3")
ENT.TauntSounds = {
	Sound("npc_dagoth/pieceofcake.mp3"),
}
local chaseMusic = Sound("npc_dagoth/panic.mp3")
...
local npc_dagoth_acquire_distance =
	CreateConVar("npc_dagoth_acquire_distance", 2500, FCVAR_NONE,
	"The maximum distance at which dagoth will chase a target.")

local npc_dagoth_spawn_protect =
	CreateConVar("npc_dagoth_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, dagoth will not target players or hide within 200 units of \z
	a spawn point.")

local npc_dagoth_attack_distance =
	CreateConVar("npc_dagoth_attack_distance", 80, FCVAR_NONE,
	"The reach of dagoth's attack.")

local npc_dagoth_attack_interval =
	CreateConVar("npc_dagoth_attack_interval", 0.2, FCVAR_NONE,
	"The delay between dagoth's attacks.")

local npc_dagoth_attack_force =
	CreateConVar("npc_dagoth_attack_force", 800, FCVAR_NONE,
	"The physical force of dagoth's attack. Higher values throw things \z
	farther.")

local npc_dagoth_smash_props =
	CreateConVar("npc_dagoth_smash_props", 1, FCVAR_NONE,
	"If set to 1, dagoth will punch through any props placed in their way.")

local npc_dagoth_allow_jump =
	CreateConVar("npc_dagoth_allow_jump", 1, FCVAR_NONE,
	"If set to 1, dagoth will be able to jump.")

local npc_dagoth_hiding_scan_interval =
	CreateConVar("npc_dagoth_hiding_scan_interval", 3, FCVAR_NONE,
	"dagoth will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant dagoths are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_dagoth_hiding_repath_interval =
	CreateConVar("npc_dagoth_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to dagoth's hiding spot will be redetermined every X seconds.")

local npc_dagoth_chase_repath_interval =
	CreateConVar("npc_dagoth_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of dagoth's target will be redetermined every \z
	X seconds.")

local npc_dagoth_expensive_scan_interval =
	CreateConVar("npc_dagoth_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_dagoth_force_download =
	CreateConVar("npc_dagoth_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download dagoth resources \z
...
if npc_dagoth_force_download:GetBool() then
	resource.AddWorkshop(workshopID)
end

util.AddNetworkString("dagoth_nag")
util.AddNetworkString("dagoth_navgen")
...
local VECTOR_dagoth_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_dagoth_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_dagoth: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other dagoths, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_dagoth"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "dagothMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_dagoth" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why dagoth isn't working.
	net.Start("dagoth_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_dagoth: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_dagoth: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_dagoth: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_dagoth: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_dagoth: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, dagoth in pairs(ents.FindByClass("npc_dagoth")) do
		dagoth:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "dagothNavGen", navEndGenerate)
	else
		print("npc_dagoth: nav_generate failed to initialize")
...
concommand.Add("npc_dagoth_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_dagoth: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. dagoth doesn't use
	...
	elseif isConsole then
		print("npc_dagoth: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_dagoth_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_dagoth_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_dagoth_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_dagoth_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. dagoth means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_dagoth_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_dagoth_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_dagoth_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_dagoth_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_dagoth_attack_distance:GetInt()
		...
		local repathInterval = npc_dagoth_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_dagoth_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_dagoth_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_dagoth_hiding_repath_interval:GetFloat()
...
local MAT_dagoth = Material("npc_dagoth/dagoth")
killicon.Add("npc_dagoth", "npc_dagoth/killicon", color_white)
language.Add("npc_dagoth", "dagoth ")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_dagoth: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a dagoth.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_dagoth_music_volume =
	CreateConVar("npc_dagoth_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by dagoth. (0-1, where 0 is muted)")

-- If another dagoth comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, dagoths do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_dagoth_PANIC_COUNT dagoths are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of dagoth.
local MUSIC_dagoth_PANIC_COUNT = 8

local MUSIC_dagoth_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_dagoth_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_dagoth") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("dagothPanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_dagoth_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_dagoth" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_dagoth_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear dagoth taunt us.
...
local function startTimer()
	if not timer.Exists("dagothPanicMusicUpdate") then
		timer.Create("dagothPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_dagoth)

	-- Get the normal vector from dagoth to the player's eyes, and then compute
	...
	-- dagoth should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("dagothHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("dagothHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("dagoth is studying this map.", "dagothHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "dagothHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "dagothHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "dagothHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "dagothHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("dagoth_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "dagothNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. dagoth doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_dagoth_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for dagoth to figure \z
		this map out.\n\z
...
net.Receive("dagoth_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! dagoth doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet dagoth-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! dagoth doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to dagoth.\n\z
			\n\z
			If you ARE the server host, you can run npc_dagoth_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not dagoth-compatible!",
			"Ok", nil,
